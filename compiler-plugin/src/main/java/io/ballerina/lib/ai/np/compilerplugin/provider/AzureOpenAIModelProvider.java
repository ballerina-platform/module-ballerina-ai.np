...
public class AzureOpenAIModelProvider implements ModelProvider {
    ...
    public GeneratedCode generateFunction(HttpClient client, String useCase, JsonArray sourceFiles)
        ...
        String responseBody = sendRequest(client, endPointUrl, constructCodeGenerationPayload(
    ...
    private String repairCode(HttpClient client, JsonObject payload)
            throws IOException, InterruptedException {
        String responseBody = sendRequest(client, endPointUrl, payload.toString(), headers);
        return ProviderUtils.getOpenAIResponseTextFromBody(responseBody);
    }

    private JsonObject constructCodeGenerationPayload(String useCase, JsonArray sourceFiles, JsonObject systemPrompt) {
        JsonObject payload = new JsonObject();
        payload.addProperty(MAX_TOKENS, 4096 * 4);
        payload.addProperty(TEMPERATURE, 0);

        JsonArray messagesArray = new JsonArray();
        messagesArray.add(systemPrompt);
